...
let mic, outputFile, micStream, rl, recordingTimeout; // Variables for microphone, output file, microphone stream, readline interface, and recording timeout
...
const startRecording = () => {
  ...
  micStream.on("data", (data) => {
    ...
    clearTimeout(recordingTimeout); // Reset the recording timeout on new data
    recordingTimeout = setTimeout(stopRecording, 4000); // Set a new recording timeout
  });
...
app.post("/AI", async (req, res) => {
  try {
    ...
    startRecording(); // Start recording immediately after AI response finishes
  } catch (error) {
    console.error("Error:", error.message);
